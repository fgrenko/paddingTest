...
def modelStuff(X_train_padded, X_test_padded, X_val_padded, y_train, y_val):
...
def regularPadding(fold, X_train, X_val, X_test, y_train, y_val, y_test, padding_type):
    ...
    y_pred, y_pred_binary = modelStuff(X_train_padded=X_train_padded, X_test_padded=X_test_padded, X_val_padded=X_val_padded, y_train=y_train, y_val=y_val)
...
for i in range(len(xDataSet)):
    ...
    print("POST obican ")
    acc, mcc, f1, gm = regularPadding(i, X_train, X_val, X_test, y_train, y_val, y_test, 'post')
    no_stop_signal_post_acc.append(acc)
    print("PRE obican ")
    acc, mcc, f1, gm = regularPadding(i, X_train, X_val, X_test, y_train, y_val, y_test, 'pre')
    no_stop_signal_pre_acc.append(acc)
